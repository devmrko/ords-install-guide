...
--   - template 'search' POST  → 텍스트 쿼리 → 벡터거리 top-k
--   - template ''       GET   → 모듈 사용법 안내(JSON)
--   - template 'list'   GET   → 적재된 문서 목록 (collection feed)
--
-- 호출 URL (LB 종단 가정):
--   POST  https://<host>/ords/vector/docs/search   body: {"q": "...", "k": 5}
...
--     &VECTOR_MODEL_NAME 가 PL/SQL 본문에 박힌 채로 ORDS 메타에 저장됨
--     (런타임에 model name 이 식별자로 그대로 쓰임)
-- ============================================================
set serveroutput on
set echo on
set define on
...
begin
  ords.define_module(
    ...
    p_items_per_page => 25,
    p_status         => 'PUBLISHED',
    p_comments       => 'Vector similarity search demo over doc_chunks'
  );
  commit;
end;
/
...
begin
  ords.define_template(
    ...
    p_pattern     => 'search',
    p_priority    => 0,
    p_comments    => 'POST {q, k} -> top-k similar docs'
  );
...
begin
  if l_body is null or dbms_lob.getlength(l_body) = 0 then
    ...
    htp.p('{"error":"empty body. expected {q, k}"}');
    return;
  end if;
  ...
  l_qvec := vector_embedding(&VECTOR_MODEL_NAME using l_q as data);
  ...
  select json_object(
    ...
    from (
      select id, title, content,
             vector_distance(embedding, l_qvec, cosine) as distance
        from doc_chunks
...
begin
  ...
  htp.p('  "endpoints": [');
  htp.p('    {"method": "POST", "path": "search", "body": {"q": "text", "k": 5}, "desc": "top-k similar docs"},');
...
begin
  ...
  ords.define_handler(
    ...
    p_items_per_page => 50,
    p_source         => q'~
      select id, title, vector_dimension_count(embedding) as dim, created_at
        from doc_chunks
       order by id
    ~'
  );
