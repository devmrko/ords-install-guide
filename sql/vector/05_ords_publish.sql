...
--
-- 임베딩 호출: DBMS_VECTOR.UTL_TO_EMBEDDING (provider=ocigenai)
--
...
begin
  ords.define_module(
    ...
    p_items_per_page => 25,
    p_status         => 'PUBLISHED',
    p_comments       => 'Vector similarity search demo over doc_chunks (Cohere v4 via OCI GenAI)'
  );
  commit;
end;
/
...
begin
  ...
  ords.define_handler(
    ...
    p_comments       => 'JSON in, JSON out. Uses Cohere embed v4 via OCI GenAI.',
    p_source         => q'~
declare
...
begin
  ...
  l_params :=
    ...
    '}';
  l_qvec := dbms_vector.utl_to_embedding(l_q, json(l_params));
...
begin
  ...
  htp.p('  "embedding": "cohere.embed-v4.0 via OCI GenAI (DBMS_VECTOR.UTL_TO_EMBEDDING)",');
  htp.p('  "endpoints": [');
