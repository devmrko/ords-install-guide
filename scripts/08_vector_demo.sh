...
#   - VECTOR_DEMO 사용자 / DOC_CHUNKS 테이블 / DOC_MODEL ONNX 임베딩 모델
...
#   ./run.sh vector-demo onnx       # 모델 로드만
#   ./run.sh vector-demo schema     # 03/04/05/06 만
...
require_env ADB_TNS WALLET_PATH \
            ...
            VECTOR_MODEL_NAME VECTOR_MODEL_URI VECTOR_MODEL_FILE
...
# DEFINE 값을 안전하게 한 줄로 만들기 (값에 따옴표 들어가면 escape)
_def() {
  printf "define %s=%s\n" "$1" "$2"
}
...
case "$step" in
  ...
  onnx)    run_as_admin 02_load_onnx.sql ;;
  schema)
    run_as_demo 03_schema.sql
    run_as_demo 04_seed.sql
    run_as_demo 05_ords_publish.sql
  ...
  *)
    die "usage: $0 {all|admin|onnx|schema|publish|test|cleanup}"
    ;;
esac
